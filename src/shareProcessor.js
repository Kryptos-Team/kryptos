...
module.exports = function (logger, poolConfiguration) {
	...
	connection.info(function (error, response) {
		...
		if (!version) {
			...
			logger.error(logSystem, logComponent, logSubcat,
				"Unsupported version: " + version + ". Minimum version required is 2.6");
		}
	});
	...
	this.handleShare = function (isValidShare, isValidBlock, shareData) {
		...
		if (isValidShare) {
			...
			logger.warning(logSystem, logComponent, logSubcat, "Invalid share by worker: " + shareData.worker);
		...
		if (isValidBlock) {
			logger.info(logSystem, logComponent, logSubcat, "Valid block found, height: " + shareData.height);
			redisCommands.push(["rename", coin + ":shares:roundCurrent", coin + ":shares:round" + shareData.height]);
			redisCommands.push(
				["sadd", coin + ":blocksPending", [shareData.blockHash, shareData.txHash, shareData.height].join(":")]);
			redisCommands.push(["hincrby", coin + ":stats", "validBlocks", 1]);
		} else if (shareData.blockHash) {
			logger.warning(logSystem, logComponent, logSubcat, "Invalid block, blockHash: " + shareData.blockHash);
