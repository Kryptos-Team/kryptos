...
const buildPoolConfigurations = function () {
    ...
    fs.readdirSync(configurationDirectory).forEach(function (file) {
        if (!fs.existsSync(configurationDirectory + file) || path.extname(configurationDirectory + file) !== '.json') return;
        ...
        for (let j = 0; j < poolConfigurationFiles.length; j++) {
            ...
            for (let k = 0; k < portsJ.length; k++) {
                if (ports.indexOf(portsJ[k]) !== -1) {
                    logger.error("Master", poolConfigurationFiles[j].fileName, "Has same configured port of " + portsJ[k] + " as " + poolConfigurationFiles[i].fileName);
                    process.exit(1);
                    return;
                }
            }
            ...
            if (poolConfigurationFiles[j].coin === poolConfigurationFiles[i].coin) {
                logger.error("Master", poolConfigurationFiles[j].fileName, "Pool has same configured coin file coins/" + poolConfigurationFiles[j].coin + " as " + poolConfigurationFiles[i].fileName + " pool");
                process.exit(1);
                return;
            }
        }
    ...
    poolConfigurationFiles.forEach(function (poolOptions) {
        ...
        if (poolOptions.coin.name in configurations) {
            logger.error("Master", poolOptions.fileName, "coins/" + poolOptions.coinFileName +
                ...
                configurations[poolOptions.coin.name].coinFileName + " used by pool configuration " +
...
const spawnPoolWorkers = function () {
    ...
    let spawnInterval = setInterval(function () {
        ...
        if (i === numForks) {
            ...
            logger.debug("Master", "PoolWorker", "Spawned " + Object.keys(poolConfigurations).length + " pool(s) on " + numForks + " thread(s)");
        }
    }, 250);
};
